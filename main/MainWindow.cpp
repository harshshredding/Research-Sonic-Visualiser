...
MainWindow::setupViewMenu()
{
    ...
    action = new QAction(tr("&Zoom custom"), this);
    action->setStatusTip(tr("some cool zooming action"));
    //connect(action, SIGNAL(triggered()), this, SLOT(zoomToRange(RealTime::fromSeconds(100),RealTime::fromSeconds(200))));
    connect(action, SIGNAL(triggered()), this, SLOT(zoomToRange()));
    connect(this, SIGNAL(canScroll(bool)), action, SLOT(setEnabled(bool)));
    menu->addAction(action);
