...
PropertyStack::repopulate()
{
    ...
    connect(box, SIGNAL(zoomButtonPressed(double,double)), this, SLOT(zoomDetected(double, double)));
...
void
PropertyStack::zoomDetected(double startTime, double endTime){
    m_client->zoomToRange(RealTime::fromSeconds(startTime), RealTime::fromSeconds(endTime));
}
