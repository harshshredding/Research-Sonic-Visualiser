...
//    m_nameWidget->setText(container->objectName());
    ...
    // add custom widget at the end
    int row = m_layout->rowCount();
    QWidget *startLabel = new QLabel(tr("Start time:"), m_mainWidget);
    QWidget *endLabel = new QLabel(tr("End time:"), m_mainWidget);
    ...
    connect(zoomButton, SIGNAL(clicked(bool)), this, SLOT(zoomSignalDetected()));



    m_layout->addWidget(startLabel, row, 0);
    m_layout->addWidget(startBox, row, 1);

    m_layout->addWidget(endLabel, row + 1, 0);
    m_layout->addWidget(endBox, row + 1, 1);
    m_layout->addWidget(zoomButton, row + 2, 0);
                                  ...
                                  bool rangeChanged)
{
    ...
    case PropertyContainer::RangeInputProperty:
    {
        groupLayout->setColumnStretch(0, 1);
        groupLayout->setColumnStretch(1, 1);
        groupLayout->setColumnStretch(2, 1);
        QWidget *startLabel = new QLabel(tr("Start Freq:"), m_mainWidget);
        QWidget *endLabel = new QLabel(tr("End Freq:"), m_mainWidget);

        startFrequencyBox = new QComboBox;
        startFrequencyBox->setEditable(true);
        endFrequencyBox = new QComboBox;
        endFrequencyBox->setEditable(true);
        QPushButton *zoomButton = new QPushButton(tr("&Zoom"), m_mainWidget);
        QPushButton *prevButton = new QPushButton(tr("&<"), m_mainWidget);
        QPushButton *fwdButton = new QPushButton(tr("&>"), m_mainWidget);

        connect(zoomButton, SIGNAL(clicked(bool)), this, SLOT(zoomFrequencySignalDetected()));
        connect(prevButton, SIGNAL(clicked(bool)), this, SLOT(previousZoomSignalDetected()));
        connect(fwdButton, SIGNAL(clicked(bool)), this, SLOT(forwardZoomSignalDetected()));

        groupLayout->addWidget(startLabel, 0, 0);
        groupLayout->addWidget(startFrequencyBox, 0, 1);
        groupLayout->addWidget(endLabel, 0 + 1, 0);
        groupLayout->addWidget(endFrequencyBox, 0 + 1, 1);

        zoomButton->setMaximumWidth(60);
        prevButton->setMaximumWidth(60);
        fwdButton->setMaximumWidth(60);
        groupLayout->addWidget(zoomButton, 0 + 2, 0);
        groupLayout->addWidget(prevButton, 2, 1);
        groupLayout->addWidget(fwdButton, 2, 2);
...
void
PropertyBox::zoomFrequencySignalDetected(){
    int startFrequency = startFrequencyBox->currentText().toInt();
    int endFrequency = endFrequencyBox->currentText().toInt();
    SpectrumLayer *layer;
...
void
PropertyBox::previousZoomSignalDetected(){
    SpectrumLayer *layer;
    if (layer = qobject_cast<SpectrumLayer *>(m_container)) {
        layer->goToPreviousZoom();
    }
}

void
PropertyBox::forwardZoomSignalDetected(){
    SpectrumLayer *layer;
    if (layer = qobject_cast<SpectrumLayer *>(m_container)) {
        layer->goToForwardZoom();
    }
}

void
PropertyBox::receiveEndingFrequencyFromView(int end){
    endFrequencyBox->setCurrentText(QString::number(end));
}

void
PropertyBox::receiveStartingFrequencyFromView(int start){
    startFrequencyBox->setCurrentText(QString::number(start));
}

/*public char[4] intToString(int num){
    char string[4];
    for(int i = 3; i >= 0; i--){
...
void
PropertyBox::zoomSignalDetected(){
    double startTime = timeToSecs(startBox->currentText());

    double endTime = timeToSecs(endBox->currentText());
    emit zoomButtonPressed(startTime, endTime);
}

double PropertyBox::timeToSecs(QString time){
    ...
    for(int i = 0; i < time.size(); i++){
        if (time.at(i) == QChar(':')){
            list.push_front(result.toDouble());
            result = "";
        }else{
            result += time.at(i);
        }
