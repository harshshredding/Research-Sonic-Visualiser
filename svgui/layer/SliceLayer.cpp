                                     ...
                                     int &minbin, int &maxbin, int &range) const
{
    ...
    minbin = int(getBinForX(v, p.x())) + m_startBin;
    maxbin = int(getBinForX(v, p.x() + 1)) + m_startBin;
...
SliceLayer::getXForBin(const LayerGeometryProvider *v, double bin) const
{
    ...
    double count = m_maxbin - m_minbin - m_startBin - m_endBin; // value changed
    if (count < 0) count = 0;
    ...
    case LogBins:
        x = w * (log10(bin +1+ m_startBin)-log10(m_startBin + 1))/(log10(m_maxbin - m_endBin + 1)-log10(m_startBin + 1));
        //x = w*log10(bin + 1)/log10(count);

        break;

    case InvertedLogBins:
       // x = w - (w * log10(count - bin - 1)) / log10(count);
        x = w - w*(log10(m_maxbin - m_endBin - bin - 1) - log10(m_startBin + 1))/(log10(m_maxbin - m_endBin + 1)-log10(m_startBin + 1));
        break;
    }
...
SliceLayer::getBinForX(const LayerGeometryProvider *v, double x) const
{
    ...
    double count = m_maxbin - m_minbin - m_startBin - m_endBin;
    if (count < 0) count = 0;
    ...
    case LinearBins:
        bin = (x * 1.0 *count)/w + eps;
        break;

    case LogBins:
        //bin = pow(10.0, (x * log10(count + 1)) / w) - 1.0 + eps;
        bin = pow(10.0, (x * (log10(m_maxbin - m_endBin + 1)-log10(m_startBin + 1)) / w)  +log10(m_startBin + 1))- 1.0 - m_startBin+ eps;
        break;

    case InvertedLogBins:
        //bin = count + 1 - pow(10.0, (log10(count) * (w - x)) / double(w)) + eps;
        bin = (-1)*(pow(10.0, (1-x/w)*(log10(m_maxbin - m_endBin + 1)-log10(m_startBin + 1))+ log10(m_startBin + 1)) - m_maxbin + m_endBin + 1);
        break;
    }
...
//    cerr << "resolution " << res << ", col0 " << col0 << ", col1 " << col1 << ", f0 " << f0 << ", f1 " << f1 << endl;
    ...
    for (int bin = 0; bin < mh - m_endBin - m_startBin; ++bin) { // value changed
        double x = nx;
        nx = getXForBin(v, bin + bin0 + 1);

        double value = m_values[bin + m_startBin];
        double norm = 0.0;
