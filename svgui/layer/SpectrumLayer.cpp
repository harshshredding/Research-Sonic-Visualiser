...
SpectrumLayer::getProperties() const
{
    ...
    list.push_back("Frequency Range");
    return list;
}
...
SpectrumLayer::getPropertyLabel(const PropertyName &name) const
{
    ...
    if (name == "Frequency Range") return tr("Range(Hz)");
...
SpectrumLayer::getPropertyType(const PropertyName &name) const
{
    ...
    if (name == "Frequency Range") return RangeInputProperty;
...
SpectrumLayer::setWindowSize(int ws)
{
    ...
    std::stack<int> tempStack1;
    std::stack<int> tempStack2;
    while(!m_prevStack1.empty()){
        int startFrequency = m_prevStack1.top()*m_sliceableModel->getSampleRate()/m_windowSize;
        int newStartBin = startFrequency*ws/m_sliceableModel->getSampleRate();
        tempStack1.push(newStartBin);
        m_prevStack1.pop();

        int endFrequency = (m_maxbin - m_prevStack2.top())*m_sliceableModel->getSampleRate()/m_windowSize;
        int newPreEndBin = endFrequency*ws/m_sliceableModel->getSampleRate();
        int oldMaxFrequency = m_maxbin*m_sliceableModel->getSampleRate()/m_windowSize;
        int newMaxBin = oldMaxFrequency*ws/m_sliceableModel->getSampleRate();
        int newEndBin = newMaxBin - newPreEndBin;
        tempStack2.push(newEndBin);
        m_prevStack2.pop();
    }

    while(!tempStack1.empty()){
        m_prevStack1.push(tempStack1.top());
        tempStack1.pop();
        m_prevStack2.push(tempStack2.top());
        tempStack2.pop();
    }

    while(!m_forwardStack1.empty()){
        int startFrequency = m_forwardStack1.top()*m_sliceableModel->getSampleRate()/m_windowSize;
        int newStartBin = startFrequency*ws/m_sliceableModel->getSampleRate();
        tempStack1.push(newStartBin);
        m_forwardStack1.pop();

        int endFrequency = (m_maxbin - m_forwardStack2.top())*m_sliceableModel->getSampleRate()/m_windowSize;
        int newPreEndBin = endFrequency*ws/m_sliceableModel->getSampleRate();
        int oldMaxFrequency = m_maxbin*m_sliceableModel->getSampleRate()/m_windowSize;
        int newMaxBin = oldMaxFrequency*ws/m_sliceableModel->getSampleRate();
        int newEndBin = newMaxBin - newPreEndBin;
    ...
    while(!tempStack1.empty()){
        m_forwardStack1.push(tempStack1.top());
        tempStack1.pop();
        m_forwardStack2.push(tempStack2.top());
        tempStack2.pop();
    }



   // int startFrequency = m_startBin*m_sliceableModel->getSampleRate()/m_windowSize;
   // int newStartBin = startFrequency*ws/m_sliceableModel->getSampleRate();
    if(!m_prevStack1.empty()){
    m_startBin = m_prevStack1.top();


   // int endFrequency = (m_maxbin - m_endBin)*m_sliceableModel->getSampleRate()/m_windowSize;
   // int newPreEndBin = endFrequency*ws/m_sliceableModel->getSampleRate();
   // int oldMaxFrequency = m_maxbin*m_sliceableModel->getSampleRate()/m_windowSize;
   // int newMaxBin = oldMaxFrequency*ws/m_sliceableModel->getSampleRate();
    m_endBin = m_prevStack2.top();
    }
    ...
    while(!m_forwardStack2.empty()){
        m_forwardStack2.pop();
    }


    m_prevStack1.push(m_startBin);
    m_prevStack2.push(m_endBin);
*/
    m_windowSize = ws;
    m_newFFTNeeded = true;
...
/*
void
SpectrumLayer::emptyStack(std::stack<int>* stack){
    while(!stack->empty()){
        stack->pop();
    }
}
...
void
SpectrumLayer::setFrequenciesForZoom(int start, int end){
    if(start > end){
        int temp  = start;
        start = end;
        end = temp;
    ...
    int candidateStartBin = start * m_windowSize/(m_sliceableModel->getSampleRate()) - m_minbin;
    if(candidateStartBin < 0){
        m_startBin = 0;
    }else if (candidateStartBin > m_maxbin){
        m_startBin = m_maxbin;
    }else{
        m_startBin = candidateStartBin;
    }


    int candidateEndBin = m_maxbin - end * m_windowSize/(m_sliceableModel->getSampleRate());
    if(candidateEndBin < 0){
        m_endBin = 0;
    }else if(candidateEndBin > m_maxbin){
        m_endBin = m_maxbin;
    }else{
        m_endBin = candidateEndBin;
    }
...
void
SpectrumLayer::goToPreviousZoom(){
    if(m_prevStack1.size() > 1){
        m_forwardStack1.push(m_prevStack1.top());
        m_forwardStack2.push(m_prevStack2.top());
        m_prevStack1.pop();
        m_prevStack2.pop();
        m_startBin = m_prevStack1.top();
        m_endBin = m_prevStack2.top();
...
void
SpectrumLayer::goToForwardZoom(){
    if(!m_forwardStack1.empty()){
        m_prevStack1.push(m_forwardStack1.top());
        m_prevStack2.push(m_forwardStack2.top());
        m_forwardStack1.pop();
        m_forwardStack2.pop();
...
SpectrumLayer::getFrequencyForX(const LayerGeometryProvider *v, double x) const
{
    ...
    double bin = getBinForX(v, x) + m_startBin;
    //return int(lrint((bin * m_sliceableModel->getSampleRate()) /
      //               m_windowSize));

    //return int(lrint((bin * m_sliceableModel->getSampleRate()) /m_windowSize)) ;
    return round(bin * m_sliceableModel->getSampleRate() /m_windowSize);
