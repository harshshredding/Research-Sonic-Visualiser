...
void
View::zoomToRange(RealTime startTime, RealTime endTime){

    sv_frame_t startFrame = RealTime::realTime2Frame(startTime, getModelsSampleRate());
    sv_frame_t endFrame = RealTime::realTime2Frame(endTime, getModelsSampleRate());
    startFrame = (endFrame - (endFrame-startFrame)/0.97);
    sv_frame_t centreFrame = (startFrame + endFrame)/2;
    setZoomLevel(((centreFrame - startFrame)/width())*2);
    setCentreFrame(centreFrame);
}
...
//              << r.width() << "x" << r.height() << ")" << endl;
    ...
    emit sendStartingFrequency((int)v0);
    ...
    emit sendEndingFrequency((int)v1);
